...
let login = () => {
    console.log("\n=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=\n");
    console.log("Please submit your Admnistrator Username and Password");
    inquirer.prompt([
        {
            type: "confirm",
            message: "Ready? \n",
    ...
    let viewLow = () => {
        console.log("\n=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=--\n");
        // console.log("Low inventory criteria:\n \n1. Magic Wand, Light Saber, Food Replicator: less than 10; \n2. Matter Transporter: less than 8; \n3. Science Tricorder: less than 50.");
        console.log("\n=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=--\n");
        console.log("Department: Imagination\n")
        connection.query("SELECT * FROM products WHERE department_name = \"Imagination\" AND stock_quantity < 10", function (err, res) {
            if (err) throw err;
            if (res.lengths === 0) {
    ...
    let viewAll = () => {
        ...
        connection.query("SELECT * FROM products WHERE stock_quantity < 6", function (err, res1) {
            if (err) throw err;
            if (res1.length === 0) {
                console.log("All items are in sufficient quantity at this time.");
            }
